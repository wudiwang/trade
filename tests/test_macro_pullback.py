...
from app.engine.macro_pullback import detect_macro_pullback, _find_spring
...
def spring_then_second_buy():
    vals = [
        ...
        (100.4, 100.45, 100.2, 100.3, 110),  # 被L2那根包含, 去包含后并入中间合并K
        (100.3, 100.5, 100.25, 100.46, 110),  # 右【合并K】(分型确认K), high=100.5
        (100.46, 100.7, 100.4, 100.6, 110),  # stall bar: 收盘100.6 > 右合并K高100.5
        (100.6, 100.8, 100.5, 100.7, 110),  # entry bar after stall
    ]
...
def body_reclaim_spring_then_second_buy():
    vals = [
        ...
        (100.3, 100.5, 100.25, 100.46, 110),   # 右合并K, high=100.5
        (100.46, 100.7, 100.4, 100.6, 110),    # stall: 收盘>100.5
        (100.6, 100.8, 100.5, 100.7, 110),     # entry
    ]
    return [k(*row, t=i) for i, row in enumerate(vals)]
...
def utad_then_second_sell():
    vals = [
        ...
        (102, 102.4, 102.1, 102.2, 110),  # 被H2那根包含, 去包含后并入中间合并K
        (102.2, 102.3, 101.9, 102.0, 110),  # 右【合并K】(分型确认K), low=101.9
        (102.0, 102.05, 101.6, 101.7, 110),  # stall bar: 收盘101.7 < 右合并K低101.9
        (101.7, 101.8, 101.4, 101.5, 110),  # entry bar after stall
    ]
    return [k(*row, t=i) for i, row in enumerate(vals)]
...
def invalid_mid_leg_spring_then_second_buy():
    vals = [
        (111, 112, 109, 110, 100),
        (109, 110, 106, 107, 100),
        (106, 107, 103, 104, 100),
        (103, 104, 100, 101, 100),
        (100, 101, 98, 99, 100),
        (99, 100, 94, 95, 360),       # local sweep, but not the completed down-leg bottom
        (95, 101, 95, 99, 130),       # body reclaim
        (99, 100, 93, 94, 100),
        (94, 98, 91, 92, 100),
        (92, 96, 90, 95, 100),        # later lower low, so the volume L1 is mid-leg
        (95, 102, 94, 101, 120),
        (101, 107, 100, 106, 120),
        (106, 106, 102, 103, 100),
        (103, 104, 100.6, 101, 90),
        (101, 103, 100.2, 100.6, 90),
        (100.6, 102, 100.0, 100.4, 90),
        (100.4, 100.45, 100.2, 100.3, 110),
...
def sweep_before_true_l1_then_second_buy():
    vals = [
        ...
        (104.9, 105.2, 104.6, 105.0, 100),   # 右合并K(分型确认K), high=105.0
        (105.0, 105.6, 104.9, 105.4, 100),   # stall: 收盘105.4 > 105.0
        (105.4, 105.8, 105.1, 105.6, 100),   # entry
    ]
    return [k(*row, t=i) for i, row in enumerate(vals)]
...
def test_detect_second_buy_requires_prior_high_volume_spring():
    ...
    assert sig.extra["structure"]["stall_time"] == spring_then_second_buy()[-2]["open_time"]
...
def test_detect_second_sell_requires_prior_high_volume_utad():
    ...
    assert sig.extra["structure"]["stall_time"] == utad_then_second_sell()[-2]["open_time"]
...
def test_second_buy_enters_on_bar_after_stall_not_stall_bar():
    assert detect_macro_pullback("SOLUSDT", "long", second_buy_without_entry_bar_after_stall(), second_buy_without_entry_bar_after_stall(), cfg()) is None
